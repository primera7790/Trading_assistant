...
def get_trade_data(html_name='index.html', iter_on=None):
    ...
    if trade_percent is not None and trade_volume is not None and trade_commission is not None:
        ...
        if current_trade_id == previous_trade_id:
            if iter_on is None:
                mb.showwarning('Сделка не найдена', emojize(':collision: Новых сделок не обнаружено. Возможны неполадки на сайте.'))
            return
    ...
    if iter_on is None:
        mb.showwarning('Нет значений новой сделки', 'Данные не подгрузились. Повторяю запрос...')
        time.sleep(10)
        return 'no data'
    else:
        return
